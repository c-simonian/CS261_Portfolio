...
# Assignment: 6
# Due Date: 8/15/2023
# Description: HashMap class using quadratic probing. Class contains methods:
# put, quadratic_prob_helper, table_load, empty_buckets,  resize_table, get
# contains_key, remove, clear, get_keys_and_values, and two dunder methods __iter__
# and __next__
...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        while self._buckets.get_at_index(index) is not None and self._buckets.get_at_index(index).is_tombstone is False:
            # if the key at the index is == key passed as param then just set the value to the value passed as param
            if self._buckets.get_at_index(index).key == key:
                self._buckets.get_at_index(index).value = value
                return
            else:
                # otherwise calculate a new index
                j_counter += 1
                index = (hash_val + j_counter * j_counter) % self._capacity
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        # this avoids out of bound errors clear method does this
        self.clear()
    ...
    def remove(self, key: str) -> None:
        ...
        index = self.quadratic_prob_helper(key)
        self._buckets.get_at_index(index)
        temp = self._buckets.get_at_index(index)
        if temp.is_tombstone is False:
            temp.is_tombstone = True
            self._size = self._size - 1
            return None
        else:
            return None
